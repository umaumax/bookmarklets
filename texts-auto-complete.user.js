...
function mergeText(baseString, inputString) {
    ...
    // ベース文字列の最後から部分一致を探す
    for (let i = 1; i <= baseString.length; i++) {
                ...
                data_src.forEach((data) => {
                    if (data.startsWith(beforeCursorLine)) {
                        ...
                        for (const item of extractPhrases(data)) {
                            preferential_data_src.add(item);
                        }
        ...
        events: {
            input: {
                ...
                selection: (event) => {
                    ...
                    let cursorPosition = beforeCursor.length; //.length + selection.length; //target.selectionStart-;
...
function main() {
    ...
    let data_src = [
        ...
        "Approved from a grammatical point of view.",
        "Approve on the assumption that other comments will be addressed.",
        "Could you update the name of this variable?",
    ];
